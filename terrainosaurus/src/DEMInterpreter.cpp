...
const size_t DEMInterpreter::BUFFER_SIZE = 41;

// How big is each record type?
const size_t DEMInterpreter::CHUNK_SIZE = 1024;
...
void DEMInterpreter::parseRecordTypeA() {
    ...
    //      "The coordinates fo the quadrangle corners are ordered in a
    ...
    double deviationAngle = readDouble(24);
    cout << "Deviation angle: " << deviationAngle << endl;
    handler->deviationAngle = deviationAngle;
    ...
    handler->setGridSize(size_t(rowGuess + 1), columns);
...
void DEMInterpreter::parseRecordTypeB(index_t r, index_t c) {
    ...
    //       sequence number."
    size_t row = readInteger(6);
    size_t col = readInteger(6);
    if (row != r || col != c) {
        ostringstream ss;
...
//        cerr << std::setw(5) << row << "," << std::setw(5) << col << endl;
    ...
    size_t pRows = readInteger(6);
    size_t pCols = readInteger(6);
...
//    cout << "Start indices: " << startRow << ", " << startCol << endl;
    ...
    for (index_t row = 0; row < startRow; row++)
...
size_t DEMInterpreter::readBytes(size_t numBytes) {
...
bool DEMInterpreter::readBoolean(size_t numBytes) {
    size_t read = readBytes(numBytes);
    if (read == numBytes)   return atoi(buffer.get()) != 0;
    else                    return false;
}

// Interpret a field of a specified size as an integer
int DEMInterpreter::readInteger(size_t numBytes) {
    size_t read = readBytes(numBytes);
...
double DEMInterpreter::readDouble(size_t numBytes) {
    size_t read = readBytes(numBytes);
...
char DEMInterpreter::readChar() {
    size_t read = readBytes(1);
    if (read == 1)          return buffer[0];
    else                    return '\0';
}

// Read a specified number of chars from the file
const char * DEMInterpreter::readChars(size_t numBytes) {
    size_t read = readBytes(numBytes);
